...
log = logging.getLogger("GPT-J-Benchmark")
...
SYNTHETIC_PROMPTS = [
    "Summarize the following: The stock market experienced significant volatility today as investors reacted to new economic data.",
    "Write a summary: Scientists have discovered a new species of deep-sea fish in the Pacific Ocean.",
    "Summarize: The city council voted to approve a new public transportation plan that will expand bus routes.",
    "Brief summary: A major tech company announced plans to build a new data center, creating thousands of jobs.",
    "Summarize this: Climate researchers published findings showing accelerated ice melt in polar regions.",
    "Create a summary: The sports team won their championship game in overtime with a dramatic final play.",
    "Summarize: New health guidelines recommend increased physical activity for adults of all ages.",
    "Write a brief summary: The museum unveiled a new exhibit featuring ancient artifacts from Egypt.",
    "Summarize the article: Economic forecasts predict moderate growth for the upcoming fiscal quarter.",
    "Create a summary: The education department announced new funding for STEM programs in schools.",
]


def load_cnn_dailymail(data_dir: str, max_examples: int) -> Tuple[Optional[List], Optional[List], Dict]:
    """Load real CNN-DailyMail articles"""
    ...
    log.info(f"Loaded {total_available} articles from CNN-DailyMail")
    ...
    return articles, references, data_info
...
def get_args():
    parser = argparse.ArgumentParser(description="GPT-J Benchmark with Real/Synthetic Data")
    ...
    parser.add_argument("--4bit", dest="use_4bit", action="store_true",
                       help="Use 4-bit quantization (~6GB VRAM)")
    parser.add_argument("--8bit", dest="use_8bit", action="store_true",
                       help="Use 8-bit quantization (~12GB VRAM)")
...
def load_model(args):
    """Load GPT-J model with appropriate device configuration"""
    ...
    if args.use_4bit or args.use_8bit:
        try:
            ...
            if args.use_4bit:
                log.info("Using 4-bit quantization (~6GB VRAM)")
                ...
                log.info("Using 8-bit quantization (~12GB VRAM)")
    ...
    if args.device == "cpu":
        ...
        log.info("Loading on CPU (this will be slow)...")
    elif args.offload:
        device_map = "auto"
    ...
    try:
        ...
        log.error("  2. --4bit       : Use 4-bit quantization (~6GB VRAM)")
        log.error("  3. --8bit       : Use 8-bit quantization (~12GB VRAM)")
...
def run_benchmark(model, tokenizer, args, prompts: List[str], references: List[str] = None, data_info: Dict = None):
    """Run the benchmark"""
    log.info("=" * 60)
    log.info("Starting GPT-J Benchmark")
    log.info("=" * 60)
    ...
    if args.mlperf:
        if args.data_type == "synthetic":
            print("GPT-J BENCHMARK SUMMARY - MLPerf Settings (SYNTHETIC DATA)")
            print("⚠️  NOT COMPARABLE TO OFFICIAL MLPERF RESULTS")
        else:
            print("GPT-J BENCHMARK SUMMARY - MLPerf Compliant")
    else:
        print("GPT-J BENCHMARK SUMMARY")
    print("=" * 60)
    ...
    elif avg_tokens_per_sec > 1:
        print("Performance:        ✅ Good")
    else:
        print("Performance:        ⚠️ Slow (try --offload)")
    print("=" * 60)
    ...
    output_file = os.path.join(args.output_dir, f"gptj_benchmark_{args.data_type}_{timestamp}.json")
